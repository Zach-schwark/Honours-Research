...
def create_comparison_plot(data_path):
    ...
    #                label=f'{algorithm}_{evidence_list}',
    ...
    ax.set_title('Magnified Log-Likelihood Comparison of Various Models Increasing \n Dataset Sizes Using 5-Fold Cross Validation', 
                fontsize=14, 
                pad=20)
    ...
    ax.set_yticklabels(['3x10$^5$', '4x10$^5$', '5x10$^5$'])
    
    # Adjust layout
    plt.tight_layout()
    
    # Save plot in various formats
    plt.savefig('Graphics/results_plots/Log_Likelihood_Full_magnified.pdf', format='pdf', bbox_inches='tight', dpi=300)
    #plt.savefig('algorithm_comparison.png', format='png', bbox_inches='tight', dpi=300)
    
    return fig, ax

# Function call
create_comparison_plot('Graphics/Weights_and_biases_chart_data/log_likelihood_full.csv')
